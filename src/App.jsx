...
const IF=[
  ...
  {id:2,uid:3,cid:3,val:120,note:"Morning session ☀️",ts:N-18000000,comments:[],rx:{"💪":[2],"👏":[4]}},
];
...
const AiTab=({challenges,setChallenges,notify})=>{
  ...
  const launch=(s,i)=>{const today=new Date().toISOString().split("T")[0],end=new Date(Date.now()+7*864e5).toISOString().split("T")[0];setChallenges(c=>[...c,{id:Date.now(),title:s.title,icon:s.icon,unit:s.unit,goal:s.goal,color:s.color,desc:s.desc,active:true,startDate:today,endDate:end}]);setDone(d=>({...d,[i]:true}));notify(`🚀 "${s.title}" launched!`);};
...
// Feed
const FeedTab=({feed,setFeed,challenges,users,cu,notify,checked,setChecked})=>{
  ...
  return<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div style={{fontWeight:700,fontSize:17}}>Activity Feed</div></div>
    {!checked&&<CheckIn onDone={()=>{setChecked(true);notify("🌅 Check-in saved!");}}/>}
    {feed.map(post=>{const u=users.find(x=>x.id===post.uid)||{name:"Unknown",color:"#888"};const ch=challenges.find(x=>x.id===post.cid);if(!ch)return null;return<div key={post.id} className="card" style={{marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}><div style={{display:"flex",gap:8,alignItems:"center"}}><Av u={u} s={32}/><div><div style={{fontWeight:700,fontSize:13}}>{u.name}</div><div style={{fontSize:10,color:"#888"}}>{ago(post.ts)}</div></div></div><Pill color={ch.color} text={`${ch.icon} ${ch.title}`}/></div><div style={{display:"flex",alignItems:"baseline",gap:5,marginBottom:5}}><span style={{fontWeight:800,fontSize:26,color:ch.color}}>{post.val.toLocaleString()}</span><span style={{fontSize:12,color:"#888"}}>{ch.unit}</span><span style={{fontSize:11,color:"#555",marginLeft:"auto"}}>/ {ch.goal.toLocaleString()}</span></div><div style={{background:"#ffffff08",borderRadius:6,height:4,marginBottom:8}}><div style={{width:`${Math.min(100,(post.val/ch.goal)*100)}%`,height:"100%",background:ch.color,borderRadius:6}}/></div>{post.note&&<div style={{fontSize:12,color:"#888",marginBottom:8,fontStyle:"italic"}}>"{post.note}"</div>}<div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:8}}>{RX.map(e=>{const us=post.rx[e]||[],a=us.includes(cu.id);return<button key={e} onClick={()=>react(post.id,e)} style={{background:a?"#ffffff15":"transparent",border:`1px solid ${a?"#ffffff30":"#ffffff0f"}`,borderRadius:20,padding:"3px 8px",fontSize:12,color:a?"#e8e8f0":"#888",display:"flex",gap:3,alignItems:"center"}}>{e}{us.length>0&&<span style={{fontSize:10}}>{us.length}</span>}</button>;})}
    </div>{post.comments.map((c,i)=>{const u2=users.find(x=>x.id===c.uid)||{name:"?",color:"#888"};return<div key={i} style={{display:"flex",gap:6,marginBottom:5}}><Av u={u2} s={22}/><div style={{background:"#ffffff08",borderRadius:9,padding:"5px 9px",flex:1,fontSize:12,color:"#aaa"}}><span style={{color:u2.color,fontWeight:600}}>{u2.name?.split(" ")[0]} </span>{c.text}</div></div>;})}
    <div style={{display:"flex",gap:6,marginTop:7}}><Av u={cu} s={22}/><input placeholder="Comment…" value={ct[post.id]||""} onChange={e=>setCt(c=>({...c,[post.id]:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&comment(post.id)} style={{flex:1,fontSize:12}}/></div></div>;})}
    {feed.length===0&&<div style={{textAlign:"center",color:"#888",padding:"40px 0",fontSize:13}}>No posts yet!</div>}
  </div>;
};

// Challenges
const ChalTab=({challenges,feed,cu,onLog})=>{const a=challenges.filter(c=>c.active);return<div><div style={{fontWeight:700,fontSize:17,marginBottom:14}}>Active Challenges</div>{a.map(ch=>{const tot=feed.filter(p=>p.uid===cu.id&&p.cid===ch.id).reduce((a,p)=>a+p.val,0);const pct=Math.min(1,tot/ch.goal);return<div key={ch.id} className="card" style={{marginBottom:12,borderColor:`${ch.color}22`}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}><div><div style={{fontSize:24,marginBottom:3}}>{ch.icon}</div><div style={{fontWeight:800,fontSize:15,color:ch.color}}>{ch.title}</div><div style={{fontSize:11,color:"#888",marginTop:2}}>{ch.desc}</div></div><Ring pct={pct} color={ch.color}/></div><div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#888",marginBottom:5}}><span>{tot.toLocaleString()} / {ch.goal.toLocaleString()} {ch.unit}</span></div><div style={{background:"#ffffff08",borderRadius:6,height:5,marginBottom:12}}><div style={{width:`${pct*100}%`,height:"100%",background:ch.color,borderRadius:6}}/></div><button onClick={()=>onLog(ch)} style={{width:"100%",background:`${ch.color}18`,border:`1px solid ${ch.color}44`,color:ch.color,padding:"9px",borderRadius:9,fontWeight:700,fontSize:13}}>Log {ch.icon} Progress</button></div>;})}
...
// Admin
const AdminTab=({cu,users,setUsers,challenges,setChallenges,notify,addNotif,session})=>{
  ...
  const createCh=async()=>{
    ...
    setChF({title:"",icon:"🏃",unit:"",goal:"",color:"#6EE7B7"});
    notify("✅ Challenge created!");
    addNotif("challenge",`New challenge: ${chF.title}`);
  };

  const SECS=[["overview","Overview","🏠"],["teams","Teams","👥"],["challenges","Challenges","🏆"]];
  ...
  return<div>
    ...
    <div style={{display:"flex",gap:4,marginBottom:14}}>{SECS.map(([id,lbl,icon])=><button key={id} onClick={()=>setSec(id)} style={{padding:"7px 14px",borderRadius:10,border:`1px solid ${sec===id?"#C4B5FD55":"#ffffff15"}`,background:sec===id?"#C4B5FD18":"transparent",color:sec===id?"#C4B5FD":"#888",fontWeight:700,fontSize:12,display:"flex",gap:4,alignItems:"center"}}><span>{icon}</span>{lbl}</button>)}</div>
    ...
    {sec==="overview"&&<div>
      ...
      <div className="card"><div style={{fontWeight:700,marginBottom:9}}>Quick Actions</div><div style={{display:"flex",gap:7,flexWrap:"wrap"}}><Btn color="#93C5FD" text="👥 New Team" onClick={()=>setSec("teams")}/><Btn color="#F9A8D4" text="🏆 Challenge" onClick={()=>setSec("challenges")}/></div></div>
    </div>}
    ...
    {sec==="challenges"&&<div>
      <div className="card" style={{marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:7}}><Inp label="Unit" value={chF.unit} onChange={e=>setChF(f=>({...f,unit:e.target.value}))} style={{flex:1}}/><Inp label="Goal" value={chF.goal} onChange={e=>setChF(f=>({...f,goal:e.target.value}))} type="number" style={{flex:1}}/></div>
      ...
      {challenges.map(ch=><div key={ch.id} className="card" style={{marginBottom:8,display:"flex",gap:9,alignItems:"center",opacity:ch.active?1:.6}}><span style={{fontSize:20}}>{ch.icon}</span><div style={{flex:1}}><div style={{fontWeight:700,color:ch.color,fontSize:13}}>{ch.title}</div><div style={{fontSize:11,color:"#888"}}>{ch.goal?.toLocaleString()} {ch.unit}</div></div><button onClick={()=>setChallenges(c=>c.map(x=>x.id===ch.id?{...x,active:!x.active}:x))} style={{background:`${ch.active?"#FCD34D":"#6EE7B7"}15`,border:`1px solid ${ch.active?"#FCD34D":"#6EE7B7"}33`,color:ch.active?"#FCD34D":"#6EE7B7",borderRadius:8,padding:"4px 8px",fontSize:11,fontWeight:700}}>{ch.active?"Pause":"Resume"}</button></div>)}
    </div>}
  </div>;
};

// Log Modal
const LogModal=({onClose,challenges,cu,setFeed,setLb,notify})=>{
  const a=challenges.filter(c=>c.active);
  const [sel,setSel]=useState(a[0]);const [val,setVal]=useState("");const [note,setNote]=useState("");
  const submit=()=>{if(!sel||!val)return;const v=parseFloat(val);if(isNaN(v)||v<=0)return;setFeed(f=>[{id:Date.now(),uid:cu.id,cid:sel.id,val:v,note,ts:Date.now(),comments:[],rx:{}},...f]);setLb(l=>{const ex=l.find(e=>e.uid===cu.id);const nx=ex?l.map(e=>e.uid===cu.id?{...e,pts:e.pts+v}:e):[...l,{uid:cu.id,pts:v,str:1}];return[...nx].sort((a,b)=>b.pts-a.pts);});notify(`✅ ${sel.icon} Logged!`);onClose();};
  return<Modal onClose={onClose}><div style={{fontWeight:700,fontSize:16,marginBottom:12}}>Log Progress</div><div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:11}}>{a.map(c=><button key={c.id} onClick={()=>setSel(c)} style={{padding:"4px 9px",borderRadius:18,border:`1px solid ${sel?.id===c.id?c.color:"#ffffff22"}`,background:sel?.id===c.id?`${c.color}22`:"transparent",color:sel?.id===c.id?c.color:"#888",fontSize:11}}>{c.icon} {c.title}</button>)}</div>{sel&&<><Inp label={`Enter ${sel.unit}`} value={val} onChange={e=>setVal(e.target.value)} type="number" min="0"/><textarea placeholder="Note (optional)" value={note} onChange={e=>setNote(e.target.value)} rows={2} style={{resize:"none",marginBottom:11}}/><Btn color={sel.color} text="Post to Feed →" style={{width:"100%",padding:"11px"}} onClick={submit}/></>}</Modal>;
};
...
export default function App({ session }) {
  ...
  return<>
    ...
    {showLog&&<LogModal onClose={()=>setShowLog(null)} challenges={challenges} cu={cu} setFeed={setFeed} setLb={setLb} notify={notify}/>}
    ...
    <div style={{maxWidth:480,margin:"0 auto",minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"12px 13px 90px"}}>
        {tab==="feed"&&<><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div style={{fontWeight:700,fontSize:17}}>Activity Feed</div><Btn color="#6EE7B7" text="+ Log" onClick={()=>setShowLog(true)}/></div><FeedTab feed={feed} setFeed={setFeed} challenges={challenges} users={allUsers} cu={cu} notify={notify} checked={checked} setChecked={setChecked}/></>}
        ...
        {tab==="admin"&&cu?.is_admin&&<AdminTab cu={cu} users={allUsers} setUsers={setUsers} challenges={challenges} setChallenges={setChallenges} notify={notify} addNotif={addNotif} session={session}/>}
